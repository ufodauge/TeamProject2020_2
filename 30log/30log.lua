...
local function deep_copy(t, dest, aType)
	...
	for k, v in pairs(t) do
		if aType ~= nil and type(v) == aType then
			r[k] = (type(v) == 'table') and ((_classes[v] or _instances[v]) and v or deep_copy(v)) or v
		elseif aType == nil then
			r[k] = (type(v) == 'table') and k ~= '__index' and ((_classes[v] or _instances[v]) and v or deep_copy(v)) or v
		end
	end
	return r
end
...
local class = {isClass = function(t)
		return not (not _classes[t])
	end, isInstance = function(t)
		return not (not _instances[t])
	end}
